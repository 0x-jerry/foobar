...
def solution(n_str, base):
    n_seq = {}

    idx = 0
    n_seq[n_str] = idx

    max_iter_count = 2**63 - 1

    while True:
        idx = idx + 1

        n_str = assign_algorithm(n_str, base)

        if n_seq.get(n_str) != None:
            return idx - n_seq.get(n_str)

        n_seq[n_str] = idx

        if idx == max_iter_count:
            break

    return -1


def assign_algorithm(n_str, base):
    """
    1) Start with a random minion ID n, which is a nonnegative integer of length k in base b
    2) Define x and y as integers of length k. x has the digits of n in descending order, and y has the digits of n in ascending order
    3) Define z = x - y. Add leading zeros to z to maintain length k if necessary
    ...
    n_len = len(n_str)
    x = int("".join(sorted(n_str, reverse=True)), base)
    y = int("".join(sorted(n_str)), base)

    z = num2str(x - y, base).rjust(n_len, "0")
